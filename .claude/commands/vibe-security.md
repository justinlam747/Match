...
1. **Secrets & Environment Variables** — Scan for hardcoded API keys, tokens, or credentials. Check for secrets exposed via client-side env var prefixes (`NEXT_PUBLIC_`, `VITE_`, `EXPO_PUBLIC_`). Verify `.env` is in `.gitignore`. See `vibe-security/references/secrets-and-env.md`.

2. **Database Access Control** — Check Supabase RLS policies, Firebase Security Rules, or Convex auth guards. This is the #1 source of critical vulnerabilities in vibe-coded apps. See `vibe-security/references/database-security.md`.

3. **Authentication & Authorization** — Validate JWT handling, middleware auth, Server Action protection, and session management. See `vibe-security/references/authentication.md`.

4. **Rate Limiting & Abuse Prevention** — Ensure auth endpoints, AI calls, and expensive operations have rate limits. Verify rate limit counters can't be tampered with. See `vibe-security/references/rate-limiting.md`.

5. **Payment Security** — Check for client-side price manipulation, webhook signature verification, and subscription status validation. See `vibe-security/references/payments.md`.

6. **Mobile Security** — Verify secure token storage, API key protection via backend proxy, and deep link validation. See `vibe-security/references/mobile.md`.

7. **AI / LLM Integration** — Check for exposed AI API keys, missing usage caps, prompt injection vectors, and unsafe output rendering. See `vibe-security/references/ai-integration.md`.

8. **Deployment Configuration** — Verify production settings, security headers, source map exposure, and environment separation. See `vibe-security/references/deployment.md`.

9. **Data Access & Input Validation** — Check for SQL injection, ORM misuse, and missing input validation. See `vibe-security/references/data-access.md`.
...
- `vibe-security/references/secrets-and-env.md` — API keys, tokens, environment variable configuration, and `.gitignore` rules.
- `vibe-security/references/database-security.md` — Supabase RLS, Firebase Security Rules, and Convex auth patterns.
- `vibe-security/references/authentication.md` — JWT verification, middleware, Server Actions, and session management.
- `vibe-security/references/rate-limiting.md` — Rate limiting strategies and abuse prevention.
- `vibe-security/references/payments.md` — Stripe security, webhook verification, and price validation.
- `vibe-security/references/mobile.md` — React Native and Expo security: secure storage, API proxy, deep links.
- `vibe-security/references/ai-integration.md` — LLM API key protection, usage caps, prompt injection, and output sanitization.
- `vibe-security/references/deployment.md` — Production configuration, security headers, and environment separation.
- `vibe-security/references/data-access.md` — SQL injection prevention, ORM safety, and input validation.
