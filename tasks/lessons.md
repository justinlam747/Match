...
## PR 5 lessons (`feat/pr5-portals-ats-scrapers`: portals schema, ATS scrapers, title filter)

- **Keyword filter substring matching is a trap**: `String.includes` matches "AI" inside "maintainer", "ML" in "HTML", "HR" in "thread". For case-insensitive whole-word matching of keywords that include non-word chars like ".NET", use `(?<![A-Za-z0-9])escaped(?![A-Za-z0-9])` — plain `\b` won't work because word boundaries treat `.` as a boundary, so ".NET" would never match.
- **Scraper duplication debt noted**: `src/lib/scrapers/greenhouse.ts`, `ashby.ts`, `lever.ts` each reimplement `isObject` / `asString` and a fetch-with-timeout block. PR 6 should extract a shared `src/lib/scrapers/_shared.ts`; the extraction was attempted in PR 5 but deferred to avoid scope creep. Any future scraper should adopt the shared module from day one.
- **Don't ship seeds for unsupported enum values**: `routeAtsScrape` returns `error: "custom ATS not supported"` for `atsType: "custom"`, so the Hugging Face / Vercel seeds with that type were silently broken on every scan. If an enum value isn't wired end-to-end, either wire it or drop the seeds — don't let dead rows sit in the DB.
- **Drizzle migration regen when schema changes mid-PR**: when Agent A generated `0004` and a critic later flagged missing `titleFilterPositive/Negative` columns, the correct fix was delete the 0004 SQL + snapshot, checkout `_journal.json`, and re-run `drizzle-kit generate --name=portals`. Never hand-edit a migration to patch a missed column — this is the PR 1 Drizzle rule applied again.
- **Rate-limit helpers without a caller = caller owns pacing**: `ats-router.ts` exports `atsRateLimitDelay(provider)` + `sleep`, but `routeAtsScrape` itself doesn't pace. Document this explicitly so downstream schedulers (PR 6) know the contract — a helper that's exported but not self-applied is a trap unless its ownership is called out.
